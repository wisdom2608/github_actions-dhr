...
#Function to disploy a banner with the script name
...
#Function to handle cases when a tag already exists on this commit
...
#Incorrect version number based on input
if [[ $VERSION == 'Release' ]]
then
    VNUM1=v$((VNUM1+1))
elif [[ $VERSION == 'Hotfix' ]]
...
#Only tag if no tag already exists (also check NEEDS_TAG here)
if [ -z "NEEDS_TAG" ]; then
    handle_success
else
    handle_existing_tag
